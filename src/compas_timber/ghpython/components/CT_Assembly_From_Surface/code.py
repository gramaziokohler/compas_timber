...
class Assembly_From_Surface(component):
    def RunScript(self, surface, stud_spacing, beam_width, frame_depth, z_axis, sheeting_outside, sheeting_inside, lintel_posts, custom_dimensions):
        ...
        if not isinstance(z_axis, RhinoVector):
        ...
        if not ((beam_width and frame_depth) or custom_dimensions):
            raise ValueError("beam_width and frame_depth must be specified either directly or through custom_dimensions.")
        ...
        assembly = SurfaceAssembly(Brep.from_native(surface), stud_spacing, beam_width, frame_depth, z_axis, custom_dimensions = custom_dimensions.dims)

        return assembly.beams, assembly.rules
