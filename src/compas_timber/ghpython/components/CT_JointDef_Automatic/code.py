...
if BeamsCollection and JointRules:
    ...
    for gr in groups:
        for i in range(n - 1):
            if beams[i].attributes["group"] != gr:
                continue

            for j in range(i + 1, n):
                if beams[j].attributes["group"] != gr:
                    continue
    ...
    for beamA, beamB in connectivity["T"]:
        ...
        try:
            ...
            log_no_def.append(("T", catA, catB))
        else:
            if joint_type[0] != "T":
                log_topo_mismatch.append(
                    ("T", catA, catB, beams.index(beamA), beams.index(beamB), joint_rules_dict[key])
                )
            else:
    ...
    for beamA, beamB in connectivity["L"]:
        ...
        try:
            ...
            log_no_def.append(("L", catA, catB))
        else:
            if joint_type[0] != "L":
                log_topo_mismatch.append(
                    ("L", catA, catB, beams.index(beamA), beams.index(beamB), joint_rules_dict[key])
                )
            else:
    ...
    for beamA, beamB in connectivity["X"]:
        ...
        try:
            ...
            log_no_def.append(("X", catA, catB))
        else:
            if joint_type[0] != "X":
                log_topo_mismatch.append(
                    ("X", catA, catB, beams.index(beamA), beams.index(beamB), joint_rules_dict[key])
                )
            else:
