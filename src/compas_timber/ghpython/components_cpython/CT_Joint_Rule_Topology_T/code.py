...
class L_TopologyPlateJointRule(Grasshopper.Kernel.GH_ScriptInstance):
    def __init__(self):
        super(L_TopologyPlateJointRule, self).__init__()
        self.classes = {}
    ...
    def RunScript(self):
        if not self.joint_type:
            ...
            return TopologyRule(JointTopology.TOPO_L, PlateMiterJoint)
        else:
            ghenv.Component.Message = self.joint_type.__name__
            return TopologyRule(JointTopology.TOPO_L, self.joint_type)
