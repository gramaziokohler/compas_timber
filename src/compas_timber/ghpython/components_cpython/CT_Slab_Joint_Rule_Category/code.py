...
class CategorySlabJointRule(Grasshopper.Kernel.GH_ScriptInstance):
    def __init__(self):
        super(CategorySlabJointRule, self).__init__()
        self.topo_bools = OrderedDict([("L", False), ("T", False)])
        ...
        if self.component.Params.Output[0].NickName not in self.classes.keys():
            ...
            parsed_output = self.component.Params.Output[0].NickName.split("_")
    ...
    def RunScript(self, *args):
        if not self.joint_type:
            warning(self.component, "Select joint type and topology type from context menu (right click)")
            return None
        else:
