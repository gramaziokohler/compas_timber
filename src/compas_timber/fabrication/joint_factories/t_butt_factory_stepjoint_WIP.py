...
class TButtFactory(object):
    ...
    @staticmethod
    def calc_params_stepjoint_heel(joint, main_part, cross_part, StepDepth=15):
        """
        Calculate the parameters for a step joint.
        ...
        Returns:
        ----------
            dict: A dictionary containing the calculated parameters for the step joint

        """

        # only valid for Heel Step Joint
        HeelDepth = 0
        StepShape = "heel"
        Tenon = "no"
        TenonWidth = 0
        ...
        # finding face less facing the cross beam the least
        ...
        StrutInclination = math.degrees(cross_part.beam.centerline.direction.angle(main_part.beam.centerline.direction))

        # find StartX according to StepDepth
        blank_edge_depth = 3-(math.sin(90-StrutInclination)*main_part.beam.width/2)
        startx = blank_edge_depth/math.sin(StrutInclination)

        if joint.ends[str(main_part.key)] == "start":
            StartX = startx
        else:
            StartX = main_part.beam.blank_length - startx
    ...
    def apply_processings(cls, joint, parts):
        ...
        if joint.birdsmouth:
            ...
            joint.calc_params_stepjoint_heel()
            ref_face = main_part.beam.faces[joint.btlx_params_main["ReferencePlaneID"]]
            joint.btlx_params_main["ReferencePlaneID"] = str(main_part.reference_surface_from_beam_face(ref_face))
            main_part.processings.append(BTLxStepJoint.create_process(joint.btlx_params_main, "T-Butt Joint"))
        else:
