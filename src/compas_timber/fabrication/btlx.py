...
class BTLxPart(object):
    ...
    def attr(self):
        return {
            ...
            "QualityGrade": "",
            "Count": "1",
            "Length": "{:.{prec}f}".format(float(self.length), prec=BTLxWriter.POINT_PRECISION),
            "Height": "{:.{prec}f}".format(float(self.height), prec=BTLxWriter.POINT_PRECISION),
            "Width": "{:.{prec}f}".format(float(self.width), prec=BTLxWriter.POINT_PRECISION),
            "Weight": "0",
