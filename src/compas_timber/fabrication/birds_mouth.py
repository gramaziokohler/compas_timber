...
class BirdsMouth(BTLxProcessing):
    """Represents a Birds Mouth feature to be made on a timber element.
    ...
    @classmethod
    def from_planes_and_element(cls, planes:list[Plane], element:TimberElement, start_y:float=0.0, width:float=None, ref_side_index:int=None, **kwargs) -> "BirdsMouth":
        """Create a BirdsMouth instance from two cutting planes and the element they notch.
        ...
        element : :class:`~compas_timber.elements.Beam` or :class:`~compas_timber.elements.Plate`
            The beam or plate to be notched by this instance.
        start_y : float, optional
            The start y-coordinate of the cut in parametric space of the reference side. Default is 0.0.
        width : float, optional
            The width of the notch. Default is the full width of the element minus start_y.
        ref_side_index : int, optional
            The reference side index of the element to be notched. Default is determined from the planes.
        ...
        if ref_side_index is None:
            ref_side_index = cls._get_default_ref_side_index(planes, element)
        ref_side = element.ref_sides[ref_side_index]
        element_width = element.get_dimensions_relative_to_side(ref_side_index)[0]

        if width is None:
            width = element_width - start_y

        # determine face limits based on start_y and width
        face_limited_front = TOL.is_positive(start_y)
        face_limited_back = element_width > start_y + width

        # compute notch apex and rear point by intersecting the planes with the front and back side planes of the element
        front_side_plane = Plane.from_frame(element.front_side(ref_side_index))
        back_side_plane = Plane.from_frame(element.back_side(ref_side_index))
        ...
        orientation = cls._calculate_orientation(element, planes)
    ...
    @classmethod
    def from_shapes_and_element(cls, plane_a: Plane | Frame, plane_b: Plane | Frame, element: TimberElement, **kwargs) -> "BirdsMouth":
        """Construct a BirdsMouth process from two planes and a timber element.
        ...
        element : :class:`~compas_timber.elements.Beam` or :class:`~compas_timber.elements.Plate`
            The beam or plate to be notched.
        ...
        """
        return cls.from_planes_and_element([plane_a, plane_b], element, **kwargs)
    ...
    @staticmethod
    def _get_default_ref_side_index(planes, element):
        ...
        return min(range(4), key=lambda i: angle_vectors(avg_normal, element.ref_sides[i].normal))

    @staticmethod
    def _calculate_orientation(element, planes):
        """Orientation is START when the combined plane normals point toward the element start."""
        combined = planes[0].normal + planes[1].normal
        if dot_vectors(element.centerline.direction, combined) > 0:
    ...
    def apply(self, geometry: Brep, element: TimberElement) -> Brep:
        """Apply the feature to the timber element geometry.

        Parameters
        ----------
        geometry : :class:`~compas.geometry.Brep`
            The timber element geometry to be notched in the element's local coordinate system.
        element : :class:`compas_timber.base.TimberElement`
            The timber element that is notched by this instance.

        Raises
        ------
        :class:`~compas_timber.errors.FeatureApplicationError`
            If the cutting planes do not intersect with element geometry.
        ...
        # convert to the local coordinates of the element
        cutting_planes = [plane.transformed(element.transformation_to_local()) for plane in cutting_planes]

        # birds mouth is always a concave (v-notch) cut
        trim_volume = element.compute_elementgeometry(False)
        try:
        ...
        if len(result) != 1:
            raise FeatureApplicationError(trim_volume, geometry, "Expected a single resulting solid from the birds mouth cut, got {}.".format(len(result)))
        return result[0]
    ...
    def planes_from_params_and_element(self, element:TimberElement) -> list[Plane]:
        ...
        ridge_line = self._get_ridge_line_from_params_and_element(element)
    ...
    def _get_ridge_line_from_params_and_element(self, element:TimberElement) -> Line:
        """Calculate the ridge line created by the intersection of the two cutting planes of the BirdsMouth notch from the machining parameters and the given timber element."""
        ...
        ref_side = element.side_as_surface(self.ref_side_index)
