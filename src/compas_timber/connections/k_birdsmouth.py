...
class KBirdsmouthJoint(Joint):
    """Represents a K-Birdsmouth type joint which joins two beams, one of them at it's end (main) and the other one along it's centerline (cross).

    This joint type is compatible with beams in K topology.

    Please use `KBirdsmouth.create()` to properly create an instance of this class and associate it with a model.
