...
class PanelLLayerButtJoint(PanelJoint, PlateLButtJoint):
    ...
    def add_extensions(self):
        butts = []
        if not (
            self.cross_panel.interior_layer
            and self.main_panel.interior_layer
            and self.main_panel.core_layer
            and self.cross_panel.core_layer
            and self.cross_panel.exterior_layer
            and self.main_panel.exterior_layer
        ):
            raise ValueError("Both panels must have interior, core and exterior layers to create a layer-to-layer butt joint.")
        butts.append(
            PanelLButtJoint(
