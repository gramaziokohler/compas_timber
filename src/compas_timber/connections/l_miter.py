...
class LMiterJoint(Joint):
    ...
    To override the miter plane, register it on `beam_a` via
    :meth:`~compas_timber.base.TimberElement.add_user_ref_plane` and pass the resulting ID as `miter_plane_id`.
    ...
    miter_plane_id : int, optional
        The BTLx integer ID (>= 100) of a `user_ref_plane` registered on `beam_a`, overriding the automatically
        calculated miter plane.
    ...
    def __data__(self):
        ...
        data["miter_plane_id"] = self.miter_plane_id
    ...
    def __init__(
        ...
        clean=False,
        miter_plane_id: Optional[int] = None,
        **kwargs,
    ):
        super(LMiterJoint, self).__init__(elements=(beam_a, beam_b), **kwargs)
        self.miter_plane_id: Optional[int] = miter_plane_id
    ...
    def miter_plane(self):
        ...
        """
        if self.miter_plane_id is None:
            return None
        frame = self.beam_a.get_user_ref_plane(self.miter_plane_id)
        if frame is None:
            raise BeamJoiningError(beams=self.elements, joint=self, debug_info="No user_ref_plane with ID {} is registered on beam_a.".format(self.miter_plane_id))
