...
class OpeningDetailBase(DetailBase):
    ...
    @staticmethod
    def prepare_opening(opening, slab_populator):
        """Prepare the opening for the given slab populator."""
        pass
...
class WindowDetailB(WindowDetailBase):
    ...
    def create_joints(self, opening, slab_populator):
        """Generate the joints for WindowDetailB."""
        joints = WindowDetailBase.create_joints(opening, slab_populator)
        joints.extend([self.get_joint_from_elements(opening.sill, jack, self.rules) for jack in opening.jack_studs])
        joints.extend([self.get_joint_from_elements(jack, opening.header, self.rules) for jack in opening.jack_studs])
...
class DoorDetailBase(OpeningDetailBase):
    ...
    def _get_adjusted_door_outline(self, opening, slab_populator):
        ...
        door_index = self._get_door_segment_index(outline, slab_populator._slab.outline_a.lines[slab_index])
