...
### Added
* Added `CompositeBeam` (`compas_timber.elements.CompositeBeam`) — a `Beam` that is also a container for other elements (`Beam`, `Plate`, or any `TimberElement`). It behaves like an ordinary `Beam` towards the rest of the model — it can be extended/cut by ordinary joints just like any beam — while owning a set of child parts with their own independently fabricable geometry, added via `add_part()` before the composite is attached to a model, then registered as its children via `merge_contained_elements(model)`. Parts can be arranged however the assembler likes (a spliced beam's segments, a truss's members, the layers of a built-up cross-section) — there is no assumption that they form a single axis. `CompositeBeam.resolve_part_at(point)` returns whichever part's geometry is closest to (or contains) a given world-space point.
* Added `Joint.resolve_composite_elements()`, called by `TimberModel.process_joinery()` before any joint computes its extensions/features. Replaces any `CompositeBeam` among a joint's elements with the real part its `location` falls in, so a joint's own `add_extensions()`/`add_features()` run completely unmodified against the real, independently fabricable part instead of the composite's nominal envelope — no joint subclass needs any `CompositeBeam`-specific code. Also refreshes `self.element_guids` to match, so a serialized-and-reloaded model doesn't restore the joint pointing at the composite again.
* Added `CompositeJoint.resolve_composite_elements()`, overriding the base implementation: `CompositeJoint` delegates its actual `add_extensions()`/`add_features()` to its own `self.joints` (unregistered sub-joints), never reading its own `.elements` directly, so the base routing (which only ever touches the outer joint's `.elements`) would otherwise be a no-op — a `CompositeBeam` referenced by one of its sub-joints is now routed correctly by delegating to each sub-joint in turn.
* Added `CompositeBeam.cut_all_parts` (default `False`, included in `CompositeBeam.__data__` so it survives serialization). When `True`, a joint targeting the composite is applied to *every* one of its parts instead of just the single nearest one — the right choice for a layered/built-up cross-section, where every layer needs the same corner treatment. Implemented via `TimberModel._expand_composite_joints()`, which clones the joint once per matching pair of parts (paired by index, or repeated against a single non-composite element) and silently skips any pair the joint type doesn't apply to (e.g. a `Plate` layer paired with a `Beam`-oriented joint, which has no `centerline`). Each pair's clone is cached on the original joint and reused across repeated `process_joinery()` calls, rather than rebuilt from scratch every time — a fresh clone has no memory of what a previous, discarded clone applied, so `clear_features()`/`clear_extensions()` would have nothing to undo and features/extensions would silently accumulate on every re-run. `CompositeJoint` is passed through unexpanded — it can't be reconstructed from a bare element pair, and its sub-joints aren't reachable from the model's registered joint list.
* Formalized the `is_group_element` contract on `TimberElement` (previously an unused/vestigial flag): an element overriding it to `True` is expected to hold its children directly until attached to a model, expose them via a property that switches to `self.children` once attached, and implement a `merge_contained_elements(model)`-shaped method called explicitly right after `model.add_element(...)` — already followed by `Panel`/`Layer` and now also by `CompositeBeam`.
