...
* Added `NurbsContour.to_contour()` and the `tessellate` parameter of `FreeContour.from_nurbs_curves_and_element()`, which write the contour as straight `Line` segments for consumers that do not support NURBS segments, such as Lignocam's BtlViewer. Writing NURBS stays the default.
