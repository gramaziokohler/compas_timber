...
* `BTLxWriter` now declares BTLx version 2.3.0 instead of 2.0.0, and takes an optional `version` argument to declare a different one. The declared version and the `xsi:schemaLocation` are derived from it, so they can no longer disagree. Some consumers accept a processing only if the declared version covers it.
