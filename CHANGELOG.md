...
* `BTLxWriter` now declares BTLx version 2.3.0 instead of 2.0.0, and takes an optional `version` argument to declare a different one. The declared version and the `xsi:schemaLocation` are derived from it, so they can no longer disagree. Some consumers accept a processing only if the declared version covers it, which matters for newer features such as NURBS contour segments.
* Added `NurbsContour` (`compas_timber.fabrication.NurbsContour`) — a contour made of NURBS curves and/or straight lines, the NURBS counterpart of `Contour`. In BTLx a NURBS curve is not a processing of its own but a contour segment type next to `Line` and `Arc` (`NURBSCurveType` in BTLx 2.3.0), so a `NurbsContour` is handed to a `FreeContour` and serializes to the same `<Contour>` element, with `<NURBS>` children.
* Added `FreeContour.from_nurbs_curves_and_element()`, which builds a `FreeContour` from a `compas_brep.curves.NurbsCurve` (or a connected sequence of curves and lines) laid on one of an element's reference sides. This makes arbitrarily shaped contours on beams and plates possible.
* Added `knotvector_to_btlx()`, `knotvector_from_btlx()` and `nurbs_curve_from_btlx()` to `compas_timber.fabrication`, converting between the standard (`count + degree + 1`) knot vector and the reduced (`count + degree - 1`) form BTLx uses.
* `BTLxReader` now deserializes `<Contour>` elements containing `<NURBS>` segments into a `NurbsContour`.
* Added `NurbsContour.to_contour()` and the `tessellate` parameter of `FreeContour.from_nurbs_curves_and_element()`, which write a NURBS contour as straight `Line` segments instead, for consumers which do not support NURBS contour segments. Lignocam's BtlViewer is one: it parses the `NURBS` element but never assigns the segment an end point, so the contour collapses onto its start point rather than failing outright. The default still writes NURBS segments as the specification defines them.
* Added `split_nurbs_curve()` to `compas_timber.fabrication.btlx`, which splits a NURBS curve in two via knot insertion (Boehm), in homogeneous coordinates so that rational curves are handled correctly. `NurbsContour` uses it to split a closed curve, since BTLx has no segment which starts where it ends -- the specification requires a circle to be given as two arcs. `NurbsContour` also rejects zero-length `Line` segments.
