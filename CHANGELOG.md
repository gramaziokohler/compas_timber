...
* `TimberModel.process_joinery()` now clears every processed joint's features before recomputing them, so individual joints no longer need to clear their own previous features at the start of `add_features()`.
* Renamed `Beam.add_blank_extension()`/`Beam.remove_blank_extension()`'s `joint_key` parameter to `joint_guid` to match what is actually passed (the joint's GUID).
