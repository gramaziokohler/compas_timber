...
* Added `SequencingInput.above`, the jointed neighbours whose base sits higher than an element's, and the `clear_above` ranking term built on it. How much higher counts is `SequencingInput.support_tolerance`, defaulting to a thousandth of `SequencingInput.height` so the same shape sequences the same way in millimetres and in metres.
* Added `support_inversions` to `assembly_sequencing.compare`: jointed pairs the sequence places the higher element of first. Unlike `height_inversions` it only counts pairs that are actually joined to each other, so it does not penalise a sequence for finishing a low bay before starting a high one.

### Changed
* Changed all Brep geometry to use `compas_brep.Brep` instead of `compas.geometry.Brep`. This covers element geometry, all BTLx processings, features, fasteners, plates and panels.
* Changed the boolean operator calls on Breps (`-`, `+`) to the new `compas_timber.utils.brep_difference` `compas_timber.utils.brep_union` and `compas_timber.utils.brep_intersection` helpers, since `compas_brep` returns one Brep per resulting piece where `compas.geometry.Brep` returned a single Brep.
* `GravityStrategy` no longer takes an element out from under a jointed element that is still standing above it, wherever an alternative exists. Sorting candidates by `base_z` was not enough: when the tallest remaining element is momentarily blocked, the ranking reached straight past it and took the next-highest, which was often the post carrying it -- and read forwards that put a beam into the model before its own support. The new `clear_above` term outranks height and forbids it. It is a preference, not a filter, so a model where nothing is ever clear above still produces a sequence rather than a dead end.
* The swept extraction check in `TimberModelAdapter.path_is_clear` now runs an exact separating-axis test between the swept oriented box and the other element's oriented box, behind the existing axis-aligned bounds as a broad phase. Axis-aligned bounds around a diagonal brace enclose several times the brace's own volume, and a beam crossing the empty part of them was reported obstructed with nothing in its way. Those false vetoes landed hardest on the highest elements, which is what a bottom-up sequence needs to take off first.
* `ground_ids` now defaults its tolerance to the input's `support_tolerance` instead of a hard `1e-3`, so what counts as "resting on the ground" is scaled to the model. Feet cut to sit on one slab still differ by a fraction of a millimetre in the lowest corner of their bounding boxes; a finer tolerance grounded only whichever foot happened to be lowest, and `disconnecting_elements` then reported ordinary removals as stranding the structure. Because `stable` leads the gravity comparator, those false alarms outvoted the support precedence exactly where it was needed. On a braced canopy of 74 beams this grounded 1 element where 54 sit on the slab, and reported 9 removals as disconnecting where none are.
* The swept extraction check no longer lets contact that already exists in the assembled model veto a direction; it asks instead whether the direction drives the element further into what it is already up against. An element is bounded by its blank, so two beams that lap always overlap as boxes, and the model records a joint only for the pairs that were explicitly connected. Testing an unjointed but touching pair as though the element started clear rejected every direction for it: on a braced canopy of 74 beams, 52 unjointed pairs overlapped at rest by a median of 39 mm on a 100 x 140 section, and the search stopped after 16 elements with 58 still standing. It now sequences all 74. This is the reasoning that already skipped jointed neighbours, applied to the contact a joint does not happen to describe.
