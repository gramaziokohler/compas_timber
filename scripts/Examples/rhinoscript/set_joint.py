...
def set_T_lap_joint(mainbeam, otherbeam):
    sc.sticky['manual_joints'].append([mainbeam, crossbeam, 'T-Lap'])

def set_T_lap_joint(mainbeam, otherbeam):
    sc.sticky['manual_joints'].append([mainbeam, crossbeam, 'T-MortiseTenon'])
...
if __name__ == "__main__":
    ...
    if not max_distance: max_distance = tol
    joint_topo, [mainbeam, otherbeam], [tA,tB] = detect_connection_topology(LineA_guid,LineB_guid,max_distance)
    print "This is a %s type of joint"%joint_topo

    if joint_topo == 'T':
        joint_type = cmd_string_options("Select joint type:..", ["Butt", "Lap", "Mortise-Tenon"], 0)
        if joint_type == 'Butt':          set_T_butt_joint(mainbeam, otherbeam)
        if joint_type == 'Lap':           set_T_lap_joint(mainbeam, otherbeam)
        if joint_type == 'Mortise-Tenon': set_T_mortisetenon_joint(mainbeam, otherbeam)
    elif joint_topo == 'L':
