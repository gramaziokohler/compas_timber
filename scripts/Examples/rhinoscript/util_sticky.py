...
if keys==[]: print "the sticky is empty"
else:
    ...
    while True:
        result0 = g0.Get()
        if result0 == Rhino.Input.GetResult.Cancel: pass
        else: result0 = g0.Option().EnglishName
        break
    ...
    if result0 == 'Show':
        ...
        for k in keys: g1.AddOption(k)
        #g1.SetDefaultString(keys[0])
        #g1.AcceptNothing(True)
        while True:
            result = g1.Get()
            if result == Rhino.Input.GetResult.Cancel: pass
            else: 
        ...
        for k in ['Yes','No']: g3.AddOption(k)
