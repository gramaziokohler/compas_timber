    ...
    for beam_a in [i_beams_with_tolerance[0], flip_beam(i_beams_with_tolerance[0])]:
        for bb in i_beams_with_tolerance[1]:
            for beam_b in [bb, flip_beam(bb)]:
                ...
                assert all([v is None for v in topo_results[1:]]), "Expected no valid topology results for I_Topo beams with tolerance"
    ...
    for beam_a in [l_beams[0], flip_beam(l_beams[0])]:
        for bb in l_beams[1]:
            for beam_b in [bb, flip_beam(bb)]:
                cs = ConnectionSolver()
                topo_results = cs.find_topology(beam_a, beam_b)
                assert topo_results[0] == JointTopology.TOPO_L, "Expected I-beam joint topology"
                assert topo_results[1] == beam_a, "Expected first beam as first plate in topology result"
                assert topo_results[2] == beam_b, "Expected second beam as second plate in topology result"
                assert TOL.is_close(topo_results[3], 0.0), "Expected distance == 0"
                assert isinstance(topo_results[4], Point), "Expected location to be a Point instance"
    ...
    for beam_a in [l_beams_with_tolerance[0], flip_beam(l_beams_with_tolerance[0])]:
        for bb in l_beams_with_tolerance[1]:
            for beam_b in [bb, flip_beam(bb)]:
                cs = ConnectionSolver()
                topo_results = cs.find_topology(beam_a, beam_b)
                assert topo_results[0] != JointTopology.TOPO_L, "Expected joint topology {}".format(JointTopology.get_name(topo_results[0]))
    ...
    for beam_a in [l_beams_with_tolerance[0], flip_beam(l_beams_with_tolerance[0])]:
        for bb in l_beams_with_tolerance[1]:
            for beam_b in [bb, flip_beam(bb)]:
                ...
                assert topo_results[1] == beam_a, "Expected first beam as first plate in topology result"
                assert topo_results[2] == beam_b, "Expected second beam as second plate in topology result"
                assert TOL.is_close(topo_results[3], dist), "Expected distance == 0.01"
                assert isinstance(topo_results[4], Point), "Expected location to be a Point instance"
    ...
    for beam_a in [t_beams[0], flip_beam(t_beams[0])]:
        for bb in t_beams[1]:
            for beam_b in [bb, flip_beam(bb)]:
                cs = ConnectionSolver()
                topo_results = cs.find_topology(beam_a, beam_b)
                assert topo_results[0] == JointTopology.TOPO_T, "Expected I-beam joint topology"
                assert topo_results[1] == beam_a, "Expected first beam as first plate in topology result"
                assert topo_results[2] == beam_b, "Expected second beam as second plate in topology result"
                assert TOL.is_close(topo_results[3], 0.0), "Expected distance == 0"
                assert isinstance(topo_results[4], Point), "Expected location to be a Point instance"
    ...
    for beam_a in [t_beams_with_tolerance[0], flip_beam(t_beams_with_tolerance[0])]:
        for bb in t_beams_with_tolerance[1]:
            for beam_b in [bb, flip_beam(bb)]:
                cs = ConnectionSolver()
                topo_results = cs.find_topology(beam_a, beam_b)
                assert topo_results[0] != JointTopology.TOPO_T, "Expected joint topology {}".format(JointTopology.get_name(topo_results[0]))
    ...
    for beam_a in [t_beams_with_tolerance[0], flip_beam(t_beams_with_tolerance[0])]:
        for bb in t_beams_with_tolerance[1]:
            for beam_b in [bb, flip_beam(bb)]:
                ...
                dist = distance_segment_segment(beam_a.centerline, beam_b.centerline)
                topo_results = cs.find_topology(beam_a, beam_b, max_distance=0.02)
                assert topo_results[0] == JointTopology.TOPO_T, "Expected T-beam joint topology"
                assert topo_results[1] == beam_a, "Expected first beam as first plate in topology result"
                assert topo_results[2] == beam_b, "Expected second beam as second plate in topology result"
                assert TOL.is_close(topo_results[3], dist), "Expected distance == 0.01"
                assert isinstance(topo_results[4], Point), "Expected location to be a Point instance"
    ...
    for bb in t_beams_with_tolerance[1]:
        for beam_a in [bb, flip_beam(bb)]:
            for beam_b in [t_beams_with_tolerance[0], flip_beam(t_beams_with_tolerance[0])]:
                ...
                topo_results = cs.find_topology(beam_a, beam_b, max_distance=0.02)
                assert topo_results[0] == JointTopology.TOPO_T, "Expected T-beam joint topology"
                assert topo_results[1] == beam_b, "Expected first beam as first plate in topology result"
                assert topo_results[2] == beam_a, "Expected second beam as second plate in topology result"
                assert TOL.is_close(topo_results[3], dist), "Expected distance == 0.01"
                assert isinstance(topo_results[4], Point), "Expected location to be a Point instance"
