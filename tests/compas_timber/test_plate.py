...
def test_flat_plate_creation():
    ...
    obb_pts = set(tuple(round(c, 6) for c in pt) for pt in plate_a.obb.points)
...
def test_plate_frame():
    polyline_a = Polyline([Point(0, 0, 0), Point(0, 20, 0), Point(10, 20, 0), Point(10, 0, 0), Point(0, 0, 0)])
    plate_a = Plate.from_outline_thickness(polyline_a, 1)
    assert plate_a.frame.point == Point(0, 20, 0), "Expected plate frame point at (0,20,0)"
    assert TOL.is_allclose(plate_a.frame.xaxis, [0, -1, 0]), "Expected plate frame xaxis to be along -Y"
    assert TOL.is_allclose(plate_a.frame.yaxis, [1, 0, 0]), "Expected plate frame yaxis to be along +X"
    assert TOL.is_allclose(plate_a.frame.zaxis, [0, 0, 1]), "Expected plate frame zaxis to be along global z axis"
...
def test_plate_frame_flipped_vector():
    ...
    assert plate_a.frame.point == Point(10, 0, 0), "Expected plate frame point at (10,0,0)"
    assert TOL.is_allclose(plate_a.frame.xaxis, [-1, 0, 0]), "Expected plate frame xaxis to be along -X"
    assert TOL.is_allclose(plate_a.frame.yaxis, [0, 1, 0]), "Expected plate frame yaxis to be along +Y"
    assert TOL.is_allclose(plate_a.frame.zaxis, [0, 0, -1]), "Expected plate frame zaxis to be along -Z"
...
def test_plate_orientation_explicit_frame_flat():
    # orientation=Y on this flat plate: yaxis aligns with +world-Y, xaxis becomes +world-X
...
def test_plate_orientation_explicit_frame_sloped():
    # orientation=X on the sloped plate: yaxis aligns with +world-X, xaxis is slope direction
