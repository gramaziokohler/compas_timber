...
def test_lap_from_brep_matches_mesh(tol, straight_beam, box_volume, ref_side_index):
    ...
    from_mesh = Lap.from_volume_and_beam(box_volume.to_mesh(), straight_beam, ref_side_index=ref_side_index)

    for attr in ("start_x", "start_y", "depth", "angle", "length", "width", "lead_angle", "lead_inclination"):
...
def test_lap_from_brep_does_not_mutate_volume(straight_beam, box_volume):
    ...
    Lap.from_volume_and_beam(volume, straight_beam, ref_side_index=0)
...
def test_lap_from_brep_is_repeatable(tol, straight_beam, box_volume):
    ...
    second = Lap.from_volume_and_beam(volume, straight_beam, ref_side_index=0)

    assert tol.is_close(first.depth, second.depth)
